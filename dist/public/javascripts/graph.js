...
async function getDataFromURL(urlAddress) {
  const response = await fetch("".concat(server, "/").concat(urlAddress));
  const fetchedData = await response.json();
  return fetchedData;
}
async function retrieveInfo() {
  [songs, authors, songRelations, songArtistRelations, authorSongRelations, authorGroupRelations] = await Promise.all([getDataFromURL("songs"), getDataFromURL("authors"), getDataFromURL("song_song_relations"), getDataFromURL("song_artist_relations"), getDataFromURL("author_song_relations"), getDataFromURL("author_group_relations")]);
}
retrieveInfo().then(() => {
  const [nodes, links] = initialDataSetup(songs, authors, songRelations, songArtistRelations, authorSongRelations, authorGroupRelations);
  console.log(links);
  const preparedNodes = prepareNodes(nodes);
  createGraph(preparedNodes, links);
  console.log(preparedNodes);
});
function initialDataSetup(songs, authors, songRelations, songArtistRelations, authorSongRelations, authorGroupRelations) {
  ...
  const modifiedIDAuthors = authors.map(author => {
    return _objectSpread(_objectSpread({}, author), {}, {
  ...
  const joinedLinks = modifiedIDSongLinks.concat(modifiedIDAuthorSongLinks, modifiedIDSongAuthorLinks, modifiedIDAuthorGroupLinks);
...
function prepareNodes(nodes) {
  return preparedNodes = nodes.map(node => {
    ...
    if (node.cancion_id.includes("c")) {
      img.src = "/images/albums/".concat(node.AlbumPic);
    } else {
      img.src = "/images/artistas/".concat(node.AlbumPic);
    }
    let autorString = "";
    if (node.Autor != null) {
...
function createGraph(nodes, links) {
  ...
  const Graph = ForceGraph()(canvasSquare).graphData(gData).width(window.innerWidth).height(window.innerHeight / 1.2).nodeId("id").linkSource("IDSource").linkTarget("IDTarget").nodeRelSize(143).nodeLabel("name").nodeCanvasObject((node, ctx) => {
    // #region CTX drawing
    ...
    ctx.arcTo(node.x - width / 2, node.y - height / 2, node.x + radius - width / 2, node.y - height / 2, radius);
    ctx.stroke();
    ctx.fill();
    ctx.save();
    ...
    ctx.arcTo(startX + imgWidth, startY, startX + imgWidth, startY + radius, radius);
    ctx.lineTo(startX + imgWidth, startY + imgHheight - radius);
    ctx.arcTo(startX + imgWidth, startY + imgHheight, startX + imgWidth - radius, startY + imgHheight, radius);
    ctx.lineTo(startX + radius, startY + imgHheight);
    ctx.arcTo(startX, startY + imgHheight, startX, startY + imgHheight - radius, radius);
    ctx.lineTo(startX, startY + radius);
    ctx.arcTo(startX, startY, startX + radius, startY, radius);
    ctx.stroke();
    ctx.clip();
    ctx.drawImage(node.pic, node.x + 30 - width / 2, node.y + 80 - height / 2, imgSize, imgSize);
    ctx.restore();
    ctx.fillStyle = "black";
    ctx.textBaseline = "hanging";
    ctx.font = 'bold 48px "Poppins"';
    ctx.textAlign = "center";
    ctx.fillText(node.name, node.x + 143 - width / 2, node.y + 30 - height / 2, 246);
    ctx.fillStyle = "black";
    ...
    for (let i = 0; i < lines.length; i++) {
      ctx.fillText(lines[i], node.x + 13 - width / 2, node.y - height / 2 + 315 + 20 * i, 261);
    }
    // #endregion
  }).nodePointerAreaPaint((node, color, ctx) => {
    ctx.fillStyle = color;
    ctx.fillRect(node.x - 286 / 2, node.y - 375 / 2, 286, 375); // draw square as pointer trap
  }).linkDirectionalArrowLength(100).linkDirectionalArrowRelPos(0.95).linkLabel("Verso").linkColor(link => {
  ...
  Graph.d3Force("box", () => {
    ...
    nodes.forEach(node => {
      ...
      if (x < X_SQUARE_HALF_SIDE_NEGATIVE) node.x = X_SQUARE_HALF_SIDE_NEGATIVE + 1;
      if (y > Y_SQUARE_HALF_SIDE) node.y = Y_SQUARE_HALF_SIDE - 1;
      if (y < Y_SQUARE_HALF_SIDE_NEGATIVE) node.y = Y_SQUARE_HALF_SIDE_NEGATIVE + 1;
    });
  });
  Graph.zoom(1 / (nodes.length / 5), 1000);
  Graph.onNodeClick(node => {
    let screenCords = Graph.graph2ScreenCoords(node.x, node.y);
    if (mouseX >= screenCords.x - 286 / 2 && mouseX <= screenCords.x + 286 / 2 && mouseY >= screenCords.y - 375 / 2 && mouseY <= screenCords.y + 375 / 2) {
